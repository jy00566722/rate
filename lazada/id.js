...
let throttle_callback = _.throttle(callback,3500,false);
...
const voucher = function(){
    ...
    chrome.storage.local.get(["my_rate"],function(result){
        let rate = result.my_rate.rate_IDR;
        for(let i =0;i<a_length;i++){
            if(!a[i].innerHTML.includes('￥')){
                if(a[i].innerHTML.includes('IDR')){
                    let s = parseFloat( a[i].innerHTML.trim().substring(3).replace(/,/g,''));
                    let rmb = (s/rate).toFixed(2);
                    a[i].innerHTML = a[i].innerHTML+`<br><sub style="color:green"> ￥${rmb}</sub>`;
                }
                else{
                    let s = parseFloat( a[i].innerHTML.trim().substring(2).replace(/\./g,''));
                    let rmb = (s/rate).toFixed(2);
                    a[i].innerHTML = a[i].innerHTML+`<br><sub style="color:green"> ￥${rmb}</sub>`;                    
                }
            }
...
const getDetailsNode =function(){
    ...
    let Price = document.querySelectorAll('span[class*="price_color_orange"]')[0];
        ...
        let my_pri= 0;
        if(Price.innerHTML.includes('IDR')){
            my_pri = parseFloat(  Price.innerHTML.substring(3).replace(/,/g,''));
        }else{
            my_pri = parseFloat(  Price.innerHTML.substring(2).replace(/\./g,''));
        }
        //console.log(my_pri);
    
    chrome.storage.local.get(["my_rate"],function(result){
        let my_rate =parseFloat( result.my_rate.rate_IDR);
...
const  getDetailsOtherNode = function(){
    ...
    chrome.storage.local.get(["my_rate"],function(result){
        ...
        for(let i=0;i<a_length;i++){
            if(!a[i].innerHTML.includes(' ')){
                if(a[i].innerHTML.includes('IDR')){  
                    let my_pri = parseFloat(  a[i].innerHTML.substring(3).replace(/,/g,''));
                    a[i].innerHTML=a[i].innerHTML+`<sub style="color:green"> ￥${(my_pri/my_rate).toFixed(2)}</sub>`;
                }else{
                    let my_pri = parseFloat(  a[i].innerHTML.substring(2).replace(/\./g,''));
                    a[i].innerHTML=a[i].innerHTML+`<sub style="color:green"> ￥${(my_pri/my_rate).toFixed(2)}</sub>`;                    
                }
            }
    ...
    chrome.storage.local.get(["my_rate"],function(result){
            ...
            for(let i=0;i<c13vh6_length;i++){
                if(c13vh6[i].tagName==='SPAN' && !c13vh6[i].innerHTML.includes(' ')){
                    if(c13vh6[i].innerHTML.includes('IDR')){
                        let my_pri = parseFloat(  c13vh6[i].innerHTML.substring(3).replace(/,/g,''));
                        c13vh6[i].innerHTML=c13vh6[i].innerHTML+`<sub style="color:green"> ￥${(my_pri/my_rate).toFixed(2)}</sub>`;
                    }else{
                        let my_pri = parseFloat(  c13vh6[i].innerHTML.substring(2).replace(/\./g,''));
                        c13vh6[i].innerHTML=c13vh6[i].innerHTML+`<sub style="color:green"> ￥${(my_pri/my_rate).toFixed(2)}</sub>`;
                    }
                }
