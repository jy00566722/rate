...
chrome.runtime.onInstalled.addListener(function(e){
  ...
  if(e.reason === 'chrome_update'){
    ...
    chrome.storage.local.set({amazon_nodes},function(){console.log('amazon_nodes设置成功')})
    chrome.storage.local.set({aliexpress_nodes},function(){console.log('aliexpress_nodes设置成功')})
    chrome.storage.local.set({aliexpress_element},function(){console.log('aliexpress_element设置成功')})
})
...
async function get_amazon_nodes(){
  let url = 'https://rate.lizudi.top/v1/amazon_nodes'
  axios.get(url).then(data=>{
    if(data.status===200 && data.data && data.data.code === 20000){
      chrome.storage.local.set({'amazon_nodes':data.data.nodes},function(){console.log('在线获取amazon_nodes并设置设置成功..')})
    }
  })
}
async function get_aliexpress_nodes(){
  let url = 'https://rate.lizudi.top/v1/aliexpress_nodes'
  axios.get(url).then(data=>{
    if(data.status===200 && data.data && data.data.code === 20000){
      chrome.storage.local.set({'aliexpress_nodes':data.data.nodes},function(){console.log('在线获取aliexpress_nodes并设置设置成功..')})
    }
  })
}
async function get_aliexpress_element(){
  let url = 'https://rate.lizudi.top/v1/aliexpress_element'
  axios.get(url).then(data=>{
    if(data.status===200 && data.data && data.data.code === 20000){
      chrome.storage.local.set({'aliexpress_element':data.data.aliexpress_element},function(){console.log('在线获取aliexpress_element并设置设置成功..')})
    }
  })
}
...
get_lazada_nodes()
get_amazon_nodes()
get_aliexpress_nodes()
get_aliexpress_element()
//定时获取汇率，更新--每小时获取一次
let se = setInterval(go_get_rate, 300000);
let ss = setInterval(get_shopee_nodes, 291000)
let sl = setInterval(get_lazada_nodes, 292000)
let sa = setInterval(get_amazon_nodes, 293000)
let sali = setInterval(get_aliexpress_nodes, 293000)
let salie = setInterval(get_aliexpress_element, 291000)
