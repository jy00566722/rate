console.log('这里是ozon价格转换插件,启动,GO!!!')
...
const all=function(){
    //console.log('总回调启动');
    chrome.storage.local.get(["my_rate",'ozon_nodes','feedback_flag'],function(result){
        rate = result.my_rate[`rate_${country}`];
        ozon_nodes = result.ozon_nodes
        feedback_flag = result.feedback_flag
        foreach_nodes(ozon_nodes)
    })
}
...
const foreach_nodes = function(node_all){
    ...
    for(let node of node_all){
        ...
        if(feedback_flag){
            let n = a.length
            feedback_list.push([`feedback-ozon-${node}`,n])
        }
        if(a[0]){
